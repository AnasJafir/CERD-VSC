...
class GroqProcessor:
    def __init__(self, api_key=None, hierarchy_path='config/parsed_data.json'):
        self.api_key = api_key
    ...
    def analyze_document(self, text):
        '''Sends text to Groq API for fallback classification.'''
        context_str = '\n'.join([f'- {k}: {v}' for k, v in self.valid_codes.items()])
        # Llama-3-8B has an 8192 max context window, so we safely truncate
        truncated_text = text[:15000] 
        
        prompt = f'''
        Tu es un expert archiviste pour le Centre d'Études (CERD). Analyse le document et extrais les métadonnées EN FORMAT JSON STRICTEMENT VALIDE.
...
class AutoProcessor:
    def __init__(self, gemini_key, groq_key, mode, hierarchy_path='config/parsed_data.json'):
        self.gemini = GeminiProcessor(gemini_key, hierarchy_path) if gemini_key else None
        self.groq = GroqProcessor(groq_key, hierarchy_path)
        self.mode = mode
        
    def analyze_document(self, text):
        if "Auto" in self.mode:
            if self.gemini:
                res = self.gemini.analyze_document(text)
                if 'error' in res and ('429' in res['error'] or 'RESOURCE_EXHAUSTED' in res['error'] or 'Max retries' in res['error']):
                    print("⚠️ Gemini Quota Exceeded. Automatique Fallback vers Groq...")
                    res_hf = self.groq.analyze_document(text)
                    if 'error' not in res_hf:
                        res_hf['_fallback_used'] = True
                    return res_hf
                return res
            else:
                return self.groq.analyze_document(text)
