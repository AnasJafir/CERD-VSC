...
const startIdx = content.indexOf("<View style={{flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', marginBottom: 16, paddingHorizontal: 8}}>");
const endStr = "{theme.label}</Text>\n          </Pressable>\n        </View>";
...
if (startIdx !== -1 && endIdx !== -1) {
  ...
  content = content.replace(content.substring(startIdx, endIdx + endStr.length), newBreadcrumb);
  
  if (!content.includes('getFullBreadcrumbsForTheme')) {
      content = content.replace("import { getArticleById, getSectorById, getThemeById }", "import { getArticleById, getSectorById, getThemeById, getFullBreadcrumbsForTheme }");
  }
  fs.writeFileSync('prova-pwa/app/e6-article.tsx', content);
  console.log("Updated e6-article.tsx layout.");
} else {
